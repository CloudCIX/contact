...
class Permissions:
    ...
    def create(request: Request) -> Optional[Http403]:
        """
        The request to create a new group record is valid if:
        - The User creating a group is a self-managed Member
        """
        # The requesting User's Member is self-managed
        if not request.user.member['self_managed']:
            return Http403(error_code='contact_group_create_201')
        return None
